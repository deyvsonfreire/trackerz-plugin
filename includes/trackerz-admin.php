...
// Adiciona a página de configurações do Trackerz ao menu do WordPress.
function trackerz_register_settings_page() {
  add_options_page(
    __( 'Trackerz', 'trackerz' ),
    __( 'Trackerz', 'trackerz' ),
    'manage_options',
    'trackerz-settings',
    'trackerz_settings_page'
  );
}
add_action( 'admin_menu', 'trackerz_register_settings_page' );
...
function trackerz_register_settings() {
  ...
  // Registra o campo de endpoint da API do Facebook
  add_settings_field(
    'trackerz_facebook_api_endpoint',
    __( 'Endpoint da API do Facebook', 'trackerz' ),
    'trackerz_facebook_api_endpoint_callback',
    'trackerz_settings',
    'trackerz_general_settings'
  );
...
function trackerz_settings_page() {
  ?>
  <div class="wrap">
    <h1><?php echo esc_html( __( 'Configurações do Trackerz', 'trackerz' ) ); ?></h1>

    <form method="post" action="options.php">
      <?php
      settings_fields( 'trackerz_settings' );
      do_settings_sections( 'trackerz_settings' );
      submit_button();
      ?>
    </form>
  </div>
...
/**
 * Callback para o campo de endpoint da API do Facebook.
 *
 * @param array $args Argumentos do campo.
 */
function trackerz_facebook_api_endpoint_callback( $args ) {
  $facebook_api_endpoint = get_option( 'trackerz_facebook_api_endpoint' );
  ?>
  <input type="text" name="trackerz_settings[trackerz_facebook_api_endpoint]" value="<?php echo esc_attr( $facebook_api_endpoint ); ?>" />
  <?php
}
